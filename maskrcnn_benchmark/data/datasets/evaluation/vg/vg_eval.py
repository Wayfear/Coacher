...
def do_vg_evaluation(
    ...
    if len(cfg.MODEL.EXRERNAL_KNOWLEDGE.ZERO_SHOT_PATH) != 0:
        zeroshot_triplet = torch.load(Path("maskrcnn_benchmark/data/datasets/evaluation/vg")/Path(cfg.MODEL.EXRERNAL_KNOWLEDGE.ZERO_SHOT_PATH), map_location=torch.device("cpu"))
        zeroshot_triplet = zeroshot_triplet['zero_shot_list'].long().numpy()
    else:
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, zero_shot=False):
    """
    Returns:
    ...
    if zero_shot:
        evaluator['eval_mean_zeroshot_recall'].prepare_zeroshot(global_container, local_container)
        evaluator['eval_ng_mean_zeroshot_recall'].prepare_zeroshot(global_container, local_container)
    ...
    if zero_shot:
        evaluator['eval_mean_zeroshot_recall'].collect_mean_recall_items(global_container, local_container, mode)
        evaluator['eval_ng_mean_zeroshot_recall'].collect_mean_recall_items(global_container, local_container, mode)
        evaluator['eval_mean_recall_by_triplet'].collect_mean_recall_items(global_container, local_container, mode)
    # Zero shot Recall
