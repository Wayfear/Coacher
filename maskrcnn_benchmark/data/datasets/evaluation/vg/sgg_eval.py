...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        nogc_pred_scores = rel_scores[nogc_score_inds[:,
                                                      0], nogc_score_inds[:, 1] + 1]
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        self.zeroshot_idx = np.where(intersect_2d(
            gt_triplets, zeroshot_triplets).sum(-1) > 0)[0].tolist()
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_zeroshot_recall']:
            ...
            if len(self.zeroshot_idx) > 0:
                ...
                # match_list = set(match_list).intersection(set(self.zeroshot_idx))
                # if len(match_list) > 0:

                #     gt_rels = local_container['gt_triplets'][np.array(list(match_list)).astype(int)]
                #     self.result_dict[mode + '_zeroshot_recall'][k].append(gt_rels)

                zeroshot_match = len(
                    self.zeroshot_idx) + len(match_list) - len(set(self.zeroshot_idx + match_list))
                zero_rec_i = float(zeroshot_match) / \
                    float(len(self.zeroshot_idx))
                self.result_dict[mode
                                 + '_zeroshot_recall'][k].append(zero_rec_i)
        ...
        for k in self.result_dict[mode + '_ng_zeroshot_recall']:
            ...
            if len(self.zeroshot_idx) > 0:
                ...
                zero_rec_i = float(zeroshot_match) / \
                    float(len(self.zeroshot_idx))
                self.result_dict[mode
                                 + '_ng_zeroshot_recall'][k].append(zero_rec_i)
...
class SGPairAccuracy(SceneGraphEvaluation):
    ...
    def prepare_gtpair(self, local_container):
        pred_pair_idx = local_container['pred_rel_inds'][:,
                                                         0] * 1024 + local_container['pred_rel_inds'][:, 1]
        gt_pair_idx = local_container['gt_rels'][:,
                                                 0] * 1024 + local_container['gt_rels'][:, 1]
        ...
        for k in self.result_dict[mode + '_accuracy_hit']:
            ...
            if mode != 'sgdet':
                ...
                if len(gt_pair_pred_to_gt) > 0:
                    ...
                    gt_pair_match = []
                self.result_dict[mode
                                 + '_accuracy_hit'][k].append(float(len(gt_pair_match)))
                self.result_dict[mode
                                 + '_accuracy_count'][k].append(float(gt_rels.shape[0]))
...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + '_mean_recall_collect'] = {20: [[] for i in range(
            self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_mean_recall'].items():
            ...
            for idx in range(num_rel_no_bg):
                ...
                self.result_dict[mode
                                 + '_mean_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode
                             + '_mean_recall'][k] = sum_recall / float(num_rel_no_bg)
        return
...
class ZSMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(ZSMeanRecall, self).__init__(result_dict)
        self.num_rel = num_rel
        self.print_detail = print_detail
        self.rel_name_list = ind_to_predicates[1:]  # remove __background__
    ...
    def register_container(self, mode):
        ...
        #self.result_dict[mode + '_recall_count'] = {20: [0]*self.num_rel, 50: [0]*self.num_rel, 100: [0]*self.num_rel}
        self.result_dict[mode
                         + '_zs_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_zs_mean_recall_collect'] = {20: [[] for i in range(
            self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode
                         + '_zs_mean_recall_list'] = {20: [], 50: [], 100: []}
    ...
    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_zs_mean_recall'].items():
            result_str += '   mR @ %d: %.4f; ' % (k, float(v))
        result_str += ' for mode=%s, type=Mean Recall.' % mode
        result_str += '\n'
        if self.print_detail:
            result_str += '----------------------- Details ------------------------\n'
            for n, r in zip(self.rel_name_list, self.result_dict[mode + '_zs_mean_recall_list'][100]):
                result_str += '({}:{:.4f}) '.format(str(n), r)
            result_str += '\n'
            result_str += '--------------------------------------------------------\n'
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        pred_to_gt = local_container['pred_to_gt']
        gt_rels = local_container['gt_rels']
        zero_short_gt = local_container['gt_rels'][self.zeroshot_idx]

        for k in self.result_dict[mode + '_zs_mean_recall_collect']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            gt_rel_sets = set(self.zeroshot_idx)
            zero_shot_match = [m for m in match if int(m) in gt_rel_sets]
            # NOTE: by kaihua, calculate Mean Recall for each category independently
            # this metric is proposed by: CVPR 2019 oral paper "Learning to Compose Dynamic Tree Structures for Visual Contexts"
            if len(zero_shot_match) > 0:
                recall_hit = [0] * self.num_rel
                recall_count = [0] * self.num_rel
                for idx in range(zero_short_gt.shape[0]):
                    local_label = zero_short_gt[idx, 2]
                    recall_count[int(local_label)] += 1
                    recall_count[0] += 1

                for idx in range(len(zero_shot_match)):
                    local_label = gt_rels[int(zero_shot_match[idx]), 2]
                    recall_hit[int(local_label)] += 1
                    recall_hit[0] += 1

                for n in range(self.num_rel):
                    if recall_count[n] > 0:
                        self.result_dict[mode + '_zs_mean_recall_collect'][k][n].append(
                            float(recall_hit[n] / recall_count[n]))
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_zs_mean_recall'].items():
            ...
            num_rel_no_bg = self.num_rel - 1
            for idx in range(num_rel_no_bg):
                if len(self.result_dict[mode + '_zs_mean_recall_collect'][k][idx + 1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(
                        self.result_dict[mode + '_zs_mean_recall_collect'][k][idx + 1])
                self.result_dict[mode
                                 + '_zs_mean_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode
                             + '_zs_mean_recall'][k] = sum_recall / float(num_rel_no_bg)
        return


"""
No Graph Constraint Zero Shot Mean Recall
"""


class SGNGZSMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(SGNGZSMeanRecall, self).__init__(result_dict)
        self.num_rel = num_rel
        self.print_detail = print_detail
        self.rel_name_list = ind_to_predicates[1:]  # remove __background__
    ...
    def register_container(self, mode):
        self.result_dict[mode
                         + '_zs_ng_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_zs_ng_mean_recall_collect'] = {20: [[] for i in range(
            self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode
                         + '_zs_ng_mean_recall_list'] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_zs_ng_mean_recall'].items():
            result_str += 'ng-mR @ %d: %.4f; ' % (k, float(v))
        result_str += ' for mode=%s, type=No Graph Constraint Mean Recall.' % mode
        result_str += '\n'
        if self.print_detail:
            result_str += '----------------------- Details ------------------------\n'
            for n, r in zip(self.rel_name_list, self.result_dict[mode + '_zs_ng_mean_recall_list'][100]):
                result_str += '({}:{:.4f}) '.format(str(n), r)
            result_str += '\n'
            result_str += '--------------------------------------------------------\n'
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_zs_ng_mean_recall_collect']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            gt_rel_sets = set(self.zeroshot_idx)
            zero_shot_match = [m for m in match if int(m) in gt_rel_sets]
            if len(zero_shot_match) > 0:
                ...
                for n in range(self.num_rel):
                    if recall_count[n] > 0:
                        self.result_dict[mode + '_zs_ng_mean_recall_collect'][k][n].append(
                            float(recall_hit[n] / recall_count[n]))
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_zs_ng_mean_recall'].items():
            ...
            num_rel_no_bg = self.num_rel - 1
            for idx in range(num_rel_no_bg):
                if len(self.result_dict[mode + '_zs_ng_mean_recall_collect'][k][idx + 1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(
                        self.result_dict[mode + '_zs_ng_mean_recall_collect'][k][idx + 1])
                self.result_dict[mode
                                 + '_zs_ng_mean_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode
                             + '_zs_ng_mean_recall'][k] = sum_recall / float(num_rel_no_bg)
        return
...
class SGNGMeanRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode
                         + '_ng_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_ng_mean_recall_collect'] = {20: [[] for i in range(
            self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode
                         + '_ng_mean_recall_list'] = {20: [], 50: [], 100: []}
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_ng_mean_recall'].items():
            ...
            for idx in range(num_rel_no_bg):
                ...
                self.result_dict[mode
                                 + '_ng_mean_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode
                             + '_ng_mean_recall'][k] = sum_recall / float(num_rel_no_bg)
        return
...
class SGAccumulateRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode
                         + '_accumulate_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
    ...
    def calculate_accumulate(self, mode):
        for k, v in self.result_dict[mode + '_accumulate_recall'].items():
            self.result_dict[mode + '_accumulate_recall'][k] = float(
                self.result_dict[mode + '_recall_hit'][k][0]) / float(self.result_dict[mode + '_recall_count'][k][0] + 1e-10)
...
def _triplet(relations, classes, boxes, predicate_scores=None, class_scores=None):
    ...
    """
    sub_id, ob_id, pred_label = relations[:,
                                          0], relations[:, 1], relations[:, 2]
...
"""
Zero shot mean recall by triplets
"""


class MeanRecallByTriplet(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(MeanRecallByTriplet, self).__init__(result_dict)
        self.num_rel = num_rel
        self.print_detail = print_detail
        self.rel_name_list = ind_to_predicates[1:]  # remove __background__
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + '_mean_recall_collect_by_triplets'] = {
            20: defaultdict(list), 50: defaultdict(list), 100: defaultdict(list)}

    def save(self, mode, file_name):

        for k, v in self.result_dict[mode + '_mean_recall_collect_by_triplets'].items():
            for t, l in v.items():
                v[t] = np.mean(np.array(l))
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_mean_recall_collect_by_triplets']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])

            recall_hit = Counter()
            recall_count = Counter()
            for idx in range(gt_rels.shape[0]):
                local_label = tuple(gt_rels[idx])
                recall_count[local_label] += 1

            for idx in range(len(match)):
                local_label = tuple(gt_rels[int(match[idx])])
                recall_hit[local_label] += 1

            for n in gt_rels:
                n = tuple(n)
                if recall_count[n] > 0:
                    self.result_dict[mode + '_mean_recall_collect_by_triplets'][k][n].append(
                        float(recall_hit[n] / recall_count[n]))
