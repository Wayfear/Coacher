...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                ...
                filter_duplicate_rels=True, filter_non_overlap=True, flip_aug=False, custom_eval=False, custom_path='', zero_shot=None):
        """
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix = get_VG_statistics(img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,
                                                image_file=self.image_file, must_overlap=True, zero_shot=self.zero_shot)
        eps = 1e-3
        bg_matrix += 1
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True, zero_shot=None):
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, 
                        ...
                        filter_duplicate_rels=False, zero_shot=zero_shot)
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap, zero_shot=None):
    """
    ...
    if num_val_im > 0:
        if split == 'val':
            ...
            image_index = image_index[num_val_im:]
            if zero_shot is not None and len(zero_shot) != 0:
                zeroshot_triplet = torch.load(Path("maskrcnn_benchmark/data/datasets/evaluation/vg")/Path(zero_shot), map_location=torch.device("cpu"))
                index = zeroshot_triplet['index']
                image_index = image_index[index]
