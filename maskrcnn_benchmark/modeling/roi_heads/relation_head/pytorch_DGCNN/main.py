...
class Classifier(nn.Module):
    def __init__(self, latent_dim, out_dim,num_node_feats, num_edge_feats, sortpooling_k, conv1d_activation='ReLU'):
        super(Classifier, self).__init__()
        self.num_node_feats = num_node_feats
        self.gnn = DGCNN(latent_dim=latent_dim,
                        output_dim=out_dim,
                        num_node_feats=num_node_feats*2,
    ...
    def PrepareFeatureLabel(self, batch_graph):
        ...
        if node_tag_flag == True:
            ...
            node_tag = torch.zeros(n_nodes, self.num_node_feats)
    ...
    def forward(self, batch_graph):
        ...
        if len(feature_label) == 2:
            node_feat, edge_feat = feature_label
        else:
            node_feat = feature_label
            edge_feat = None
        embed = self.gnn(batch_graph, node_feat, edge_feat)
        return embed
