...
from .model_motifs import LSTMContext, FrequencyBias, PathContext, GNNPathContext, LabelLayer, GraphLabelLayer, TopKNeighborContext
from .model_motifs_with_attribute import AttributeLSTMContext
from .model_transformer import TransformerContext
from .utils_relation import layer_init, get_box_info, get_box_pair_info
from maskrcnn_benchmark.data import get_dataset_statistics
from .gnn import DGCNN, extract_enclosing_subgraphs
...
@registry.ROI_RELATION_PREDICTOR.register("MotifPredictorWithExternalKnowledge")
class MotifPredictorWithExternalKnowledge(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifPredictorWithExternalKnowledge, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        if config.MODEL.EXRERNAL_KNOWLEDGE.TOPK:
            self.external_knowledge_layer = TopKNeighborContext(config, obj_classes, in_channels, 5)
        else:
            self.external_knowledge_layer = PathContext(config, obj_classes, in_channels, 8)
        ...
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        self.post_emb = nn.Linear(self.hidden_dim, self.hidden_dim * 2)
        self.concat_post_cat = nn.Linear(self.hidden_dim * 2 + config.MODEL.EXRERNAL_KNOWLEDGE.PATH_DIM, self.pooling_dim)
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls, bias=True)

        # initialize layer parameters 
        layer_init(self.post_emb, 10.0 * (1.0 / self.hidden_dim) ** 0.5, normal=True)
        layer_init(self.concat_post_cat, xavier=True)
        layer_init(self.rel_compress, xavier=True)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        Returns:
            ...
            rel_pair_idxs (list[Tensor]): (num_rel, 2) index of subject and object
            union_features (Tensor): (batch_num_rel, context_pooling_dim): visual union feature of each pair
            roi_features: (batch_num_box, context_pooling_dim)
            union_features: (batch_num_rel, context_pooling_dim)
        """
        ...
        # rel_pair_idx = torch.cat(rel_pair_idxs)

        # rel_pair_pred = obj_preds[rel_pair_idx.reshape(-1)]
        # rel_pair_pred = rel_pair_pred.reshape(rel_pair_idx.shape)
        ...
        # for pred_labels, pairs in zip(obj_preds, rel_pair_idxs):
        #     rel_pair_pred = pred_labels[pairs.reshape(-1)]
        #     rel_pair_pred = rel_pair_pred.reshape(pairs.shape)
        #     pair_preds.append(rel_pair_pred)
        ...
        path_context = self.external_knowledge_layer(pair_pred, union_features)
        combine_feature = torch.cat((prod_rep,path_context), dim=1)
        prod_rep = self.concat_post_cat(combine_feature)
...
@registry.ROI_RELATION_PREDICTOR.register("PredictorWithLabelInfo")
class PredictorWithLabelInfo(nn.Module):
    def __init__(self, config, in_channels):
        super(PredictorWithLabelInfo, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.label_layer = GraphLabelLayer(config, obj_classes, in_channels)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        path_context = self.label_layer(pair_preds)
        combine_feature = torch.cat((prod_rep,path_context), dim=1)
        prod_rep = self.concat_post_cat(combine_feature)
...
@registry.ROI_RELATION_PREDICTOR.register("GNNPredictorWithExternalKnowledge")
class GNNPredictorWithExternalKnowledge(nn.Module):
    def __init__(self, config, in_channels):
        super(GNNPredictorWithExternalKnowledge, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.external_knowledge_layer = GNNPathContext(config, obj_classes, in_channels)
...
@registry.ROI_RELATION_PREDICTOR.register("RefineMotifPredictor")
class RefineMotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(RefineMotifPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.dcgnn = DGCNN(4096+self.num_obj_cls, 512, self.num_rel_cls)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        obj_dists = obj_dists.split(num_objs, dim=0)
        rel_dists = rel_dists.split(num_rels, dim=0)
        roi_featuress = roi_features.split(num_objs, dim=0)

        for i, rels in enumerate(rel_dists):
            if rels.shape[0] == 0:
                continue
            selected_edge_index = torch.argmax(rels, dim=-1, keepdim=False)
            selected_edge_index = selected_edge_index > 0
            edges = rel_pair_idxs[i][selected_edge_index]
            if edges.shape[0] > 0:
                node_feature = torch.cat((roi_featuress[i], obj_dists[i]), dim=-1)
                data_list = extract_enclosing_subgraphs(edges, edges, node_feature)
                # loader = DataLoader(data_list, batch_size=edges.shape[0])
                # b = next(iter(loader))
                batch_x = torch.cat([d.x for d in data_list], dim=0)
                batch_edge_index = torch.cat([d.edge_index for d in data_list], dim=-1)
                batch = torch.cat([torch.ones(d.x.shape[0])*i for i, d in enumerate(data_list)]).long().cuda()
                logits = self.dcgnn(batch_x, batch_edge_index, batch)
                rel_dists[i][selected_edge_index] = logits
