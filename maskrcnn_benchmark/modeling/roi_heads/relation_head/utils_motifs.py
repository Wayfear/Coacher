...
def obj_edge_vectors(names, wv_dir, wv_type='numberbatch-en', wv_dim=300):
    if wv_type == 'numberbatch-en':
        fname = os.path.join(wv_dir, wv_type + '.pt')
        wv_dict, wv_arr, wv_size = torch.load(fname, map_location=torch.device("cpu"))
        wv_dim = wv_size
    else:
