...
def make_optimizer(cfg, model, logger, slow_heads=None, slow_ratio=5.0, fast_heads=None, fast_ratio=5.0, rl_factor=1.0):
    params = []
        ...
        if slow_heads is not None:
            for item in slow_heads:
                if item in key:
                    ...
                    lr = lr / slow_ratio
                    break
        if fast_heads is not None:
            for item in fast_heads:
                if item in key:
                    logger.info("FAST HEADS: {} is faster by ratio of {}.".format(key, str(fast_ratio)))
                    lr = lr * fast_ratio
                    break
