...
def train(cfg, local_rank, distributed, logger):
    ...
    # NOTE, we slow down the LR of the layers start with the names in slow_heads
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "MotifPredictorWithExternalKnowledge":
        # fast_heads = ["roi_heads.relation.predictor.external_knowledge_layer.path_ctx_rnn"]
        fast_heads = []
    else:
        fast_heads = []
    ...
    optimizer = make_optimizer(cfg, model, logger, slow_heads=slow_heads, slow_ratio=10.0, fast_heads=fast_heads, fast_ratio=cfg.MODEL.EXRERNAL_KNOWLEDGE.FAST_RATIO, rl_factor=float(num_batch))
